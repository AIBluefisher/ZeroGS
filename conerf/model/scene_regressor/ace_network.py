...
class Head(nn.Module):
    ...
    def __init__(self,
        ...
        self.res3_conv2 = nn.Conv2d(
            self.head_channels, self.head_channels, 1, 1, 0)
        ...
        if self.use_homogeneous:
            ...
            self.register_buffer(
                "max_scale", torch.tensor([homogeneous_max_scale]))
            self.register_buffer(
                "min_scale", torch.tensor([homogeneous_min_scale]))
            self.register_buffer("max_inv_scale", 1. / self.max_scale)
            self.register_buffer("h_beta", math.log(2) /
                                 (1. - self.max_inv_scale))
...
class Regressor(nn.Module):
    ...
    def __init__(self, num_head_blocks, use_homogeneous, num_encoder_features=512):
        """
        Constructor.
    ...
    def create_from_encoder(cls, encoder_state_dict, num_head_blocks, use_homogeneous):
        ...
        # Create a regressor.
        _logger.info(
            "Creating Regressor using pretrained encoder with %d feature size.",
    ...
    def create_from_state_dict(cls, state_dict):
        ...
        # Create a regressor.
        _logger.info("Creating regressor from pretrained state_dict:"
                     "\n\tNum head blocks: %d"
